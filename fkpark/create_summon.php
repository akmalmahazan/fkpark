...
// Build QR URL
// IMPORTANT:
// - The QR must encode the RECEIPT URL (summon_view.php?id=...)
// - If you scan with a PHONE, using "localhost" / "127.0.0.1" will NOT work.
//   In that case, set $PUBLIC_BASE_URL to your PC's LAN IP, e.g. http://192.168.0.10/fkpark
$qrDataUrl = null;
$summonViewUrl = null;

// Set this if you want the QR to be scannable from other devices (phone).
// Example: $PUBLIC_BASE_URL = 'http://192.168.0.10/fkpark';
// Leave null to auto-detect using the current request host.
$PUBLIC_BASE_URL = null;

function fkpark_base_url($override = null) {
    if ($override) return rtrim($override, '/');

    $scheme = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off') ? 'https' : 'http';
    $host = $_SERVER['HTTP_HOST'] ?? 'localhost';

    // If the host is localhost, try SERVER_ADDR (may still be 127.0.0.1 on local dev)
...
if ($createdSummon) {
    $baseUrl = fkpark_base_url($PUBLIC_BASE_URL);

    // Receipt page to open after scanning
    $summonViewUrl = $baseUrl . '/summon_view.php?id=' . (int)$createdSummon['traffic_summonID'];

    // QR IMAGE that encodes the receipt URL
...
<div class="layout">
    <aside class="sidebar">
        ...
        <nav class="nav">
            ...
            <a href="student_status_security.php" class="nav-link"><span>👤</span><span>Student Status</span></a>    
            <a class="nav-link" href="security_profile.php"><span class="icon">🙍</span>Profile</a>
        </nav>
