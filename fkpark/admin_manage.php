...
// Fetch admin's actual name from database
if ($adminUsername !== '') {
    $sqlAdminName = "SELECT name FROM admin WHERE admin_id = ?";
    $stmtAdminName = mysqli_prepare($conn, $sqlAdminName);
    if ($stmtAdminName) {
        mysqli_stmt_bind_param($stmtAdminName, "s", $adminUsername);
        mysqli_stmt_execute($stmtAdminName);
        $resultAdminName = mysqli_stmt_get_result($stmtAdminName);
        if ($resultAdminName && $rowAdminName = mysqli_fetch_assoc($resultAdminName)) {
...
/* =========================================================
   5. LOAD USER LIST (with search filter)
   ========================================================= */
$users = [];
...
if ($searchQuery !== '') {
    $sqlList .= " WHERE (
        l.username LIKE ? OR
        COALESCE(s.name, se.name, a.name) LIKE ? OR
        COALESCE(s.email, se.email, a.email) LIKE ? OR
        COALESCE(s.phone, se.phone, a.phone) LIKE ?
    )";
}

$sqlList .= " ORDER BY l.username ASC";

$stmtList = mysqli_prepare($conn, $sqlList);

if ($searchQuery !== '') {
    $searchParam = '%' . $searchQuery . '%';
    mysqli_stmt_bind_param($stmtList, "ssss", $searchParam, $searchParam, $searchParam, $searchParam);
}

if ($stmtList && mysqli_stmt_execute($stmtList)) {
...
/* =========================================================
   6. IF EDIT REQUEST, LOAD THAT USER FOR MODAL
   ========================================================= */
        ...
        <main class="content">
            <div class="content-header">
                ...
                <div class="header-actions">
                    <!-- Search Bar -->
                    <form method="get" action="admin_manage.php" class="search-container">
                        ...
                        >
                        <?php if ($searchQuery !== ''): ?>
                            <a href="admin_manage.php" class="clear-search" title="Clear search">✕</a>
                        <?php endif; ?>
                    </form>
            ...
            <?php if ($searchQuery !== ''): ?>
                <div class="search-info">
                    <span>Showing search results for: <strong>"<?php echo htmlspecialchars($searchQuery); ?>"</strong> (<?php echo count($users); ?> user<?php echo count($users) !== 1 ? 's' : ''; ?> found)</span>
                    <a href="admin_manage.php">Clear search</a>
                </div>
            <?php endif; ?>
                                ...
                                <td>
                                    <div class="action-icons">
                                        <a
                                            class="action-edit"
                                            title="Edit"
                                            href="admin_manage.php?edit_username=<?php echo urlencode($user['username']); ?><?php echo $searchQuery !== '' ? '&search=' . urlencode($searchQuery) : ''; ?>#editUserModal"
                                        >✏️</a>
                                        <a
                                            class="action-delete"
                                            title="Delete"
                                            href="admin_manage.php?delete_username=<?php echo urlencode($user['username']); ?><?php echo $searchQuery !== '' ? '&search=' . urlencode($searchQuery) : ''; ?>"
...
<div class="modal-backdrop" id="addUserModal">
    <div class="modal">
        <div class="modal-header">
            ...
            <a href="admin_manage.php<?php echo $searchQuery !== '' ? '?search=' . urlencode($searchQuery) : ''; ?>" class="modal-close">&times;</a>
        </div>
        <form method="post" action="admin_manage.php<?php echo $searchQuery !== '' ? '?search=' . urlencode($searchQuery) : ''; ?>#addUserModal">
            ...
            <div class="modal-footer">
                <a href="admin_manage.php<?php echo $searchQuery !== '' ? '?search=' . urlencode($searchQuery) : ''; ?>" class="btn-secondary">Cancel</a>
...
<div class="modal-backdrop" id="editUserModal">
    <div class="modal">
        ...
        <form method="post" action="admin_manage.php?edit_username=<?php echo urlencode($editUser['username']); ?><?php echo $searchQuery !== '' ? '&search=' . urlencode($searchQuery) : ''; ?>#editUserModal">
