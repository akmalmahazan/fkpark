...
// Get cumulative student count data (total students registered over time)
$registrationData = [];

// Get all students by their registration dates (using the student table created date as proxy)
// Since student table doesn't have registration_date, we'll get the count by role_type = 'Student' in login table
$sqlGetStudents = "
    SELECT 
        s.student_id,
        s.name,
...
$resultStudents = mysqli_query($conn, $sqlGetStudents);
$totalStudents = 0;
if ($resultStudents) {
    $totalStudents = mysqli_num_rows($resultStudents);
}

// Create cumulative data for last 6 months showing growth
// We'll distribute the students across months to show growth pattern
...
for ($i = 5; $i >= 0; $i--) {
    ...
    // Calculate cumulative students for this month
    // This simulates growth - in production, you'd track actual registration dates
    if ($i == 0) {
        // Current month shows actual total
        $chartData[] = $totalStudents;
    } else {
        // Previous months show proportional growth (you can adjust this logic)
        $proportionFactor = (6 - $i) / 6.0; // Gradual increase
        $chartData[] = (int)($totalStudents * $proportionFactor);
    }
}

// Alternative: If you want actual data, track when students were added
// For now, this shows the current total prominently
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<div class="layout">
    ...
    <a href="parking_area.php" class="nav-link">
        <span class="nav-icon"><i class="bi bi-p-square"></i></span>
    ...
    <div class="main">
        ...
        <main class="content">
            ...
            <!-- NEW: Total Students Registered Chart -->
            <div class="chart-section">
                <div class="chart-header">
                    <div>
                        <div class="chart-title">Total Students Registered</div>
                        <div class="chart-subtitle">Current total: <?php echo $totalStudents; ?> students | Updates automatically when new students are added</div>
                    </div>
                </div>
...
<script>
// Initialize Chart for Total Students
const ctx = document.getElementById('studentRegistrationChart').getContext('2d');
            ...
            tooltip: {
                ...
                callbacks: {
                    label: function(context) {
                        return 'Total Students: ' + context.parsed.y;
                    }
                }
            }
        },
...
// Auto-refresh chart every 5 seconds to show updates when admin adds students
setInterval(function() {
    fetch('get_student_count.php?t=' + Date.now())
        .then(response => response.json())
        .then(data => {
            if (data.success) {
                // Update chart data
                studentChart.data.labels = data.labels;
                studentChart.data.datasets[0].data = data.values;
                studentChart.update('none'); // Update without animation for smooth refresh
                
                // Update the subtitle with new total
                const subtitle = document.querySelector('.chart-subtitle');
                if (subtitle) {
                    subtitle.textContent = 'Current total: ' + data.total + ' students | Updates automatically when new students are added';
                }
                
                // Also update the "Registered Users" stat card
                const statCards = document.querySelectorAll('.stat-card');
                statCards.forEach(card => {
                    const label = card.querySelector('.stat-label');
                    if (label && label.textContent === 'Registered Users') {
                        const valueEl = card.querySelector('.stat-value');
                        if (valueEl && valueEl.textContent !== data.totalUsers.toString()) {
                            valueEl.textContent = data.totalUsers;
                            // Flash animation
                ...
                console.log('✅ Chart updated: ' + data.total + ' students');
            }
        })
        .catch(error => console.error('Error updating chart:', error));
}, 5000); // Refresh every 5 seconds

console.log('📊 Student count chart initialized with auto-refresh');
</script>
</body>
</html>