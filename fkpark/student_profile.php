...
// Fetch profile from student table
$student = ['name'=>'', 'email'=>'', 'phone'=>''];
$stmt = $conn->prepare("SELECT name, email, phone FROM student WHERE student_id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_profile'])) {
    if (!hash_equals($_SESSION['csrf'], $_POST['csrf'] ?? '')) {
        ...
        if ($name === '' || $email === '' || $phone === '') {
            ...
            $stmt = $conn->prepare("UPDATE student SET name=?, email=?, phone=? WHERE student_id=?");
            $stmt->bind_param("ssss", $name, $email, $phone, $studentUsername);
            if ($stmt->execute()) {
    ...
    if (!hash_equals($_SESSION['csrf'], $_POST['csrf'] ?? '')) {
        ...
        if ($new1 !== $new2) {
            ...
            if (!$hash || !password_verify($current, $hash)) {
                ...
                $stmt->bind_param("ss", $newHash, $studentUsername);
                if ($stmt->execute()) {
        ...
        <header class="topbar">
            <div class="topbar-user">
                <div class="topbar-user-info">
                    <div>Welcome, <?php echo esc($student['name'] ?: $studentUsername); ?></div>
                    <div class="topbar-role">Student</div>
                </div>
                <div class="avatar">
                    <?php echo strtoupper(substr($student['name'] ?: $studentUsername, 0, 1)); ?>
                </div>
            </div>
        </header>
        ...
        <main class="content">
            ...
            <div class="profile-header">
                <div class="profile-avatar">
                    <?php echo esc(strtoupper(substr($student['name'] ?: $studentUsername, 0, 1))); ?>
                </div>
                <div class="profile-info">
                    <h2><?php echo esc($student['name'] ?: $studentUsername); ?></h2>
                    <p>Student • ID: <?php echo esc($studentUsername); ?></p>
                </div>
            </div>
            ...
            <div class="card">
                ...
                <form method="post">
                    ...
                    <div class="form-field full">
                        ...
                        <input name="name" class="form-input" value="<?php echo esc($student['name']); ?>" required>
                    </div>

                    <div class="form-row">
                        <div class="form-field">
                            <label>Email Address</label>
                            <input name="email" type="email" class="form-input" value="<?php echo esc($student['email']); ?>" required>
                        </div>
                        <div class="form-field">
                            <label>Phone Number</label>
                            <input name="phone" class="form-input" value="<?php echo esc($student['phone']); ?>" required>
                        </div>
                    </div>
